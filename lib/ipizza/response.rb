class Ipizza::Response
  ...
  def verify(certificate_path, from_charset = 'UTF-8', to_charset = 'UTF-8', extra_params={})
    param_order = @@response_param_order[@params['VK_SERVICE']]
    verify_params = param_order.inject(Hash.new) { |h, p| h[p] = @params[p]; h }
    mac_string = Ipizza::Util.mac_data_string(verify_params, param_order, from_charset, to_charset, extra_params)
    @mac = mac_string
