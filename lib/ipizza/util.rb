...
module Ipizza
  class Util
    ...
    class << self
	  ...
	  as_two = true
	  as_two = false if extra_params['bank_type'].eql?('swedbank') unless extra_params['bank_type'].nil?
          memo << func_p(val, as_two) << val
	  @mac = memo
	  memo
        end
      end
      ...
      def func_p(val, as_two)
	  if as_two
	    return sprintf("%03i", val.bytesize)
	  else
	    return sprintf("%03i", val.size)
	  end
      end
    end
  end
